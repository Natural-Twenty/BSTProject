...
// Searches a given TfIdfList for a node with the same given filename and add
// the given tfIdfSum value to the node's tfIdfSum value.
/*
TfIdfList addTfIdfSum (TfIdfList list, char *filename, double tfIdfSum)
{
    ...
    while (curr != NULL) {
        if (strcmp(curr->filename, filename) == 0) {
            curr->tfIdfSum = curr->tfIdfSum + tfIdfSum;
            return list;
        }
        curr = curr->next;
    }
